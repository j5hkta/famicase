"""
Script de inicializacion para Railway (se ejecuta en el paso 'release' antes de arrancar el servidor).
Crea tablas, ejecuta migraciones y carga datos iniciales si la BD esta vacia.
"""
import os
...
with app.app_context():
    # Asegurar que el directorio de datos existe (volumen Railway o carpeta local)
    os.makedirs(app.config['DATA_DIR'], exist_ok=True)

    db.create_all()

    # Migracion: añadir pin_acceso si la columna no existe aun
    ...
    if not TipoTrabajo.query.first():
        ...
        db.session.commit()
        print(f"{len(tipos)} tipos de trabajo creados.")
    ...
    if not Odontologo.query.first():
        ...
        db.session.commit()
        print(f"{len(odontologos)} odontologos creados.")
